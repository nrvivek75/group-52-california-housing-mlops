...
from .linear_regression import LinearRegressionModel, RidgeRegressionModel, LassoRegressionModel
...
class ModelTrainer:
    ...
    def load_data(self):
        ...
        logger.info(f"Data loaded - Train: {self.X_train.shape}, Test: {self.X_test.shape}")
    ...
    def train_model(self, model: BaseModel) -> Dict[str, Any]:
        ...
        with self.mlflow_tracker.start_run(run_name=f"{model.name}_experiment"):
            try:
                ...
                logger.info(f"{model.name} - RMSE: {test_metrics['rmse']:.4f}, "
    ...
    def register_best_model(self, model_name: str = "california_housing_best_model"):
        ...
        if model_version:
            logger.info(f"Best model registered: {model_name}, Version: {model_version.version}")

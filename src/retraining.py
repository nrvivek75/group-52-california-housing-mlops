...
class ModelRetrainingSystem:
    ...
    def check_model_performance(self) -> Tuple[bool, Dict]:
        ...
        try:
            # Load current model
            current_model = mlflow.pyfunc.load_model(f"models:/{self.model_name}/Production")
    ...
    def check_data_drift(self, window_days: int = 30) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            logger.info(f"Data drift check completed - Recent samples: {len(recent_data)}")
    ...
    def trigger_retraining(self, trigger_type: str, force: bool = False) -> Dict:
        ...
        try:
            ...
            with mlflow.start_run(run_name=f"retraining_{trigger_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                ...
                if rmse <= self.threshold_rmse and r2 >= self.threshold_r2:
                    ...
                    logger.info(f"New model registered and deployed: {model_details.name}")
                    ...
                    logger.warning(f"Retrained model performance below thresholds: RMSE={rmse}, R2={r2}")
                    return {
